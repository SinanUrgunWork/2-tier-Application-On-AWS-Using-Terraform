...
# internet gateway oluştur ve vpc ye bağla
# internet gateway is the bridge between amazon VPC to internet
...
# data kaynağı kullanarak belirtilmiş olan bölgedeki avalability zones elde edildi
...
resource "aws_subnet" "pub_sub_1a" {
  ...
  availability_zone = data.aws_availability_zones.avalability_zones.availability_zones.name[0]
...
resource "aws_subnet" "pub_sub_2b" {
  ...
  availability_zone = data.aws_availability_zones.avalability_zones.availability_zones.name[1]
  map_public_ip_on_launch = true
...
# route table oluştur
# route tables are the set of rules that determine where network traffic from your vpc is directed
